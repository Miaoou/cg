...
{   
    ...
    while (1) {
        ...
        if( turn._roundType < 0 )
            ...
            if( dist( team, myHero._x, closestEnemy._x ) <= 125
                || dist( team, myHero._x, rangestEnemy._attackRange + rangestEnemy._x ) <= 30 )
                action::move( sub( team, myHero._x, 300 ), myHero._y );
                
            else if( allyDistance > 0 && allyDistance < 125 )
                action::attack_nearest( nextEnemy._unitType );
                
            else if( allyDistance > 0 && allyDistance >= 125 )
                //action::move_attack( closestEnemy._x - 80, closestEnemy._y, nextEnemy._unitId );
                 action::attack_nearest( nextEnemy._unitType );
                
            else
                action::move( sub( team, furthestAlly._x, 125 ), furthestAlly._y );
        }
    }
}
